...
if (isProd) {
  ...
  // Au premier lancement, si la BDD n'existe pas dans AppData, on la copie depuis le package
  if (!fs.existsSync(prodDbPath)) {
    try {
      // S'assurer que le dossier AppData pour l'app existe
      ...
      console.log('Base de données initialisée dans AppData.');
    } catch (error) {
      console.error('Erreur lors de la copie de la base de données initiale:', error);
    }
  }
}
...
function setupDatabaseIPC() {
  ...
  ipcMain.handle('db:settings:update', async (event, data) => {
    ...
    return prisma.settings.upsert({
      ...
      create: { id: 1, schoolName, schoolAddress, printerName },
    });
  });
