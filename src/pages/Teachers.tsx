...
const Teachers = () => {
  const { user } = useAuth();
  const accessLevel = getAccessLevel(user?.role, user?.permissions, PERMISSIONS.CAN_MANAGE_TEACHERS);
  ...
  const addWorkHoursMutation = useMutation({ mutationFn: addTeacherWorkHours, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["teacherWorkHours", selectedTeacher?.id] }); queryClient.invalidateQueries({ queryKey: ["teacherStats", selectedTeacher?.id] }); queryClient.invalidateQueries({ queryKey: ["todayWorkStats", selectedTeacher?.id] }); toast({ title: "Heures enregistrées" }); setOpenWorkHoursDialog(false); }, onError: (error) => { toast({ title: "Erreur", description: `Erreur: ${error.message}`, variant: "destructive" }); } });
        ...
        <Card className="md:col-span-1">
          ...
          <CardContent>
            {isLoadingTeachers ? (
              ...
              <div className="space-y-2">{teachers.map((teacher) => (<div key={teacher.id} className={`flex items-center justify-between p-3 border rounded-md cursor-pointer hover:bg-muted/50 transition-colors ${selectedTeacher?.id === teacher.id ? "bg-muted" : ""}`} onClick={() => handleTeacherSelect(teacher)}><div><h3 className="font-medium">{teacher.first_name} {teacher.name}</h3><p className="text-sm text-muted-foreground">{teacher.speciality || teacher.email}</p></div>
              {!isReadOnly && <div className="flex space-x-1"><Button size="icon" variant="ghost" onClick={(e) => { e.stopPropagation(); handleEditTeacher(teacher); }}><Edit className="h-4 w-4" /></Button><Button size="icon" variant="ghost" onClick={(e) => { e.stopPropagation(); handleDeleteTeacher(teacher.id); }}><Trash className="h-4 w-4" /></Button></div>}
              </div>))}
              </div>
            )}
          </CardContent>
            ...
            <CardContent>
              <Tabs defaultValue="overview">
                ...
                <TabsContent value="overview">
                  ...
                  <div><div className="flex justify-between items-center mb-2"><h3 className="text-lg font-medium">Répartition des heures ce mois</h3>{!isReadOnly && <Button variant="outline" size="sm" onClick={handleAddWorkHours}><PlusCircle className="h-4 w-4 mr-1" /> Ajouter des heures</Button>}</div>{isLoadingStats ? <p className="text-center py-4">Chargement...</p> : teacherStats?.subjectHours.length === 0 ? <p className="text-muted-foreground">Aucune heure enregistrée ce mois-ci</p> : <Table><TableHeader><TableRow><TableHead>Matière</TableHead><TableHead className="text-right">Heures</TableHead><TableHead className="text-right">Montant</TableHead></TableRow></TableHeader><TableBody>{teacherStats?.subjectHours.map((item, index) => (<TableRow key={index}><TableCell>{item.name}</TableCell><TableCell className="text-right">{item.hours.toFixed(1)}h</TableCell><TableCell className="text-right">{Math.round((selectedTeacher.hourlyRate || 0) * item.hours).toLocaleString()} FCFA</TableCell></TableRow>))}</TableBody></Table>}</div>
                </TabsContent>
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <Form {...teacherForm}>
            <form onSubmit={teacherForm.handleSubmit(onTeacherFormSubmit)} className="space-y-4 py-4">
              <div className="grid grid-cols-2 gap-4"><FormField control={teacherForm.control} name="firstName" render={({ field }) => (<FormItem><FormLabel>Prénom</FormLabel><FormControl><Input disabled={isReadOnly} placeholder="Prénom" {...field} /></FormControl><FormMessage /></FormItem>)} /><FormField control={teacherForm.control} name="lastName" render={({ field }) => (<FormItem><FormLabel>Nom</FormLabel><FormControl><Input disabled={isReadOnly} placeholder="Nom" {...field} /></FormControl><FormMessage /></FormItem>)} /></div>
              <FormField control={teacherForm.control} name="email" render={({ field }) => (<FormItem><FormLabel>Email</FormLabel><FormControl><Input disabled={isReadOnly} type="email" placeholder="email@example.com" {...field} /></FormControl><FormMessage /></FormItem>)} />
              <FormField control={teacherForm.control} name="phone" render={({ field }) => (<FormItem><FormLabel>Téléphone</FormLabel><FormControl><Input disabled={isReadOnly} placeholder="Numéro de téléphone" {...field} /></FormControl><FormMessage /></FormItem>)} />
              <FormField control={teacherForm.control} name="password" render={({ field }) => (<FormItem><FormLabel>Mot de passe</FormLabel><FormControl><Input disabled={isReadOnly} type="password" placeholder={selectedTeacher ? "Laisser vide pour ne pas changer" : "Mot de passe obligatoire"} {...field} /></FormControl><FormMessage /></FormItem>)} />
              <FormField control={teacherForm.control} name="address" render={({ field }) => (<FormItem><FormLabel>Adresse</FormLabel><FormControl><Input disabled={isReadOnly} placeholder="Adresse du professeur" {...field} /></FormControl><FormMessage /></FormItem>)} />
              <FormField control={teacherForm.control} name="speciality" render={({ field }) => (<FormItem><FormLabel>Spécialité</FormLabel><FormControl><Input disabled={isReadOnly} placeholder="Spécialité ou domaine d'expertise" {...field} /></FormControl><FormMessage /></FormItem>)} />
              <FormField control={teacherForm.control} name="hourlyRate" render={({ field }) => (<FormItem><FormLabel>Taux horaire (FCFA)</FormLabel><FormControl><Input disabled={isReadOnly} type="number" placeholder="Taux horaire" {...field} /></FormControl><FormMessage /></FormItem>)} />
              {!isReadOnly && <DialogFooter><Button type="button" variant="outline" onClick={() => setOpenTeacherDialog(false)}>Annuler</Button><Button type="submit"><Save className="h-4 w-4 mr-2" />Enregistrer</Button></DialogFooter>}
            </form>
          </Form>
        </DialogContent>
      </Dialog>

      <Dialog open={openWorkHoursDialog} onOpenChange={setOpenWorkHoursDialog}><DialogContent className="sm:max-w-[500px]"><DialogHeader><DialogTitle>Pointer des heures de travail</DialogTitle><DialogDescription>Enregistrez les heures travaillées par {selectedTeacher?.firstName} {selectedTeacher?.lastName}</DialogDescription></DialogHeader><Form {...workHoursForm}><form onSubmit={workHoursForm.handleSubmit(onWorkHoursFormSubmit)} className="space-y-4"><FormField control={workHoursForm.control} name="date" render={({ field }) => (<FormItem><FormLabel>Date</FormLabel><FormControl><Input disabled={isReadOnly} type="date" {...field} /></FormControl><FormMessage /></FormItem>)} /><div className="grid grid-cols-2 gap-4"><FormField control={workHoursForm.control} name="start_time" render={({ field }) => (<FormItem><FormLabel>Heure de début</FormLabel><FormControl><Input disabled={isReadOnly} type="time" {...field} /></FormControl><FormMessage /></FormItem>)} /><FormField control={workHoursForm.control} name="end_time" render={({ field }) => (<FormItem><FormLabel>Heure de fin</FormLabel><FormControl><Input disabled={isReadOnly} type="time" {...field} /></FormControl><FormMessage /></FormItem>)} /></div><FormField control={workHoursForm.control} name="subjectId" render={({ field }) => (<FormItem><FormLabel>Matière (Optionnel)</FormLabel><Select disabled={isReadOnly} onValueChange={field.onChange} defaultValue={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Sélectionner une matière" /></SelectTrigger></FormControl><SelectContent>{getTeacherSubjects().map((subject) => (<SelectItem key={subject.id} value={subject.id.toString()}>{subject.name} {subject.class && `- ${subject.class.name}`}</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)} /><FormField control={workHoursForm.control} name="notes" render={({ field }) => (<FormItem><FormLabel>Notes</FormLabel><FormControl><Textarea disabled={isReadOnly} placeholder="Notes additionnelles" {...field} /></FormControl><FormMessage /></FormItem>)} />{!isReadOnly && <DialogFooter><Button type="button" variant="outline" onClick={() => setOpenWorkHoursDialog(false)}>Annuler</Button><Button type="submit">Enregistrer</Button></DialogFooter>}</form></Form></DialogContent></Dialog>
    </MainLayout>
  );
};
