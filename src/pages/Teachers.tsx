...
import { getTeachers, addTeacher, updateTeacher, deleteTeacher, getTeacherStats, getTeacherWorkHours, addTeacherWorkHours, getSubjects } from "@/lib/api";
...
const Teachers = () => {
  ...
  const { data: teacherStats, isLoading: isLoadingStats } = useQuery({ queryKey: ["teacherStats", selectedTeacher?.id], queryFn: () => selectedTeacher ? getTeacherStats(selectedTeacher.id) : Promise.resolve({ totalHoursThisMonth: 0, totalEarningsThisMonth: 0, subjectHours: [] }), enabled: !!selectedTeacher });
          ...
          {selectedTeacher ? (
            <CardContent>
              <Tabs defaultValue="overview">
                ...
                <TabsContent value="workhours"><div className="flex justify-between items-center mb-4"><h3 className="text-lg font-medium">Historique des pointages</h3><Button onClick={handleAddWorkHours}><PlusCircle className="h-4 w-4 mr-2" /> Pointer des heures</Button></div>{isLoadingWorkHours ? <p className="text-center py-4">Chargement...</p> : workHours.length === 0 ? <p className="text-center py-4">Aucun pointage enregistré pour ce professeur.</p> : <Table><TableHeader><TableRow><TableHead>Date et heure</TableHead><TableHead>Matière</TableHead><TableHead className="text-right">Heures</TableHead><TableHead>Notes</TableHead></TableRow></TableHeader><TableBody>{workHours.map((record) => (<TableRow key={record.id}><TableCell>{format(new Date(record.date), "dd/MM/yyyy")}</TableCell><TableCell>{record.start_time} - {record.end_time}</TableCell><TableCell>{getSubjectName(record.subject_id)}</TableCell><TableCell className="text-right">{record.hours.toFixed(2)}h</TableCell><TableCell>{record.notes || "-"}</TableCell></TableRow>))}</TableBody></Table>}</TabsContent>
              </Tabs>
            </CardContent>
